...
namespace YMWeb.Service.CommonService
{
    public class DatabaseTableMySqlService : RepositoryBase, IDatabaseTableService
    {
        ...
        public async Task<List<TableInfo>> GetTablePageList(string tableName, Pagination pagination)
        {
            ...
            strSql.Append(@"SELECT table_name TableName FROM information_schema.tables where table_schema='" + GetDatabase() + "'");
        ...
        /// <summary>
        /// 仅用在YMWeb框架里面，同步不同数据库之间的数据，以 MySql 为主库，同步 MySql 的数据到SqlServer和Oracle，保证各个数据库的数据是一样的
        /// </summary>
        /// <returns></returns>
        ...
        {
            string sqlServerConnectionString = "localhost;Initial Catalog = YMWebNetDb;User ID=sa;Password=1q2w3e4r;MultipleActiveResultSets=true";
        ...
        {
            string database = HtmlHelper.Resove(GlobalContext.SystemConfig.DBConnectionString.ToLower(), "initial catalog=", ";");
            return database;
        }
        #endregion
    }
