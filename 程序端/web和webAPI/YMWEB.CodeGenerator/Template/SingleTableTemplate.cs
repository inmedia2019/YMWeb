...
namespace YMWeb.CodeGenerator
{
    public class SingleTableTemplate
    {
        private string buttoncacheKey = "YMWeb_modulebuttondata_";
        private string fieldscacheKey = "YMWeb_modulefieldsdata_";
        private string cacheKey = "YMWeb_moduleldata_";
        private string quickcacheKey = "YMWeb_quickmoduledata_";
        private string initcacheKey = "YMWeb_init_";
        private string authorizecacheKey = "YMWeb_authorizeurldata_";// +权限
        ...
        public BaseConfigModel GetBaseConfig(string path,string username, string tableName, string tableDescription, Dictionary<string,string> tableFieldList)
        {
            ...
            baseConfigModel.OutputConfig.OutputEntity = Path.Combine(path, "YMWeb.Domain\\Entity");
            baseConfigModel.OutputConfig.OutputService = Path.Combine(path, "YMWeb.Service");
            baseConfigModel.OutputConfig.OutputWeb = Path.Combine(path, "YMWeb.Web");
            #endregion
        ...
        public string BuildEntity(BaseConfigModel baseConfigModel, DataTable dt,string idColumn = "F_Id")
        {
            ...
            //sb.AppendLine("using YMWeb.Code;");
            sb.AppendLine("using Chloe.Annotations;");
            sb.AppendLine();

            sb.AppendLine("namespace YMWeb.Domain." + baseConfigModel.OutputConfig.OutputModule);
            sb.AppendLine("{");
        ...
        public string BuildService(BaseConfigModel baseConfigModel, DataTable dt, string idColumn = "F_Id")
        {
            ...
            sb.AppendLine("using YMWeb.Domain." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine();

            sb.AppendLine("namespace YMWeb.Service." + baseConfigModel.OutputConfig.OutputModule);
            sb.AppendLine("{");
            ...
            sb.AppendLine("    {");                    
            sb.AppendLine("        private string cacheKey = \"YMWeb_" + baseConfigModel.FileConfig.ClassPrefix.ToLower() + "data_\";");
        ...
        public string BuildController(BaseConfigModel baseConfigModel, string idColumn="F_Id")
        {
            ...
            sb.AppendLine("using YMWeb.Code;");
            sb.AppendLine("using YMWeb.Domain." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine("using YMWeb.Service;");
            sb.AppendLine("using Microsoft.AspNetCore.Authorization;");
            sb.AppendLine("using YMWeb.Service."+ baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine();

            sb.AppendLine("namespace YMWeb.Web.Areas." + baseConfigModel.OutputConfig.OutputModule + ".Controllers");
            sb.AppendLine("{");
            ...
            sb.AppendLine("        {");
            sb.AppendLine("           " + baseConfigModel.FileConfig.EntityName+ " data = null;");
            sb.AppendLine("            if (!string.IsNullOrEmpty(keyValue))");
            sb.AppendLine("            {");
            sb.AppendLine("            data = await _service.GetForm(keyValue);");
            sb.AppendLine("            }");
            sb.AppendLine("            try");
            sb.AppendLine("            {");
            sb.AppendLine("                await _service.SubmitForm(entity, keyValue);");
            sb.AppendLine("                return await Success(\"操作成功。\", className, entity, data, keyValue);");
            sb.AppendLine("            }");
            sb.AppendLine("            catch (Exception ex)");
            sb.AppendLine("            {");
            sb.AppendLine("                return await Error(ex.Message, className, entity, data, keyValue);");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        [HttpPost]");
            ...
            sb.AppendLine("                return await Success(\"操作成功。\", className, data, data, keyValue, DbLogType.Delete);");
            sb.AppendLine("            }");
            sb.AppendLine("            catch (Exception ex)");
            sb.AppendLine("            {");
            sb.AppendLine("                return await Error(ex.Message, className, data, data, keyValue, DbLogType.Delete);");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("        #endregion");
            sb.AppendLine("    }");
        ...
        public string BuildIndex(BaseConfigModel baseConfigModel, string idColumn = "F_Id")
        {
            ...
            if (baseConfigModel.PageIndex.IsTree==1)
            {
                ...
                sb.AppendLine("             url: '@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/GetTreeGridJson'+(!queryJson ? '' : '?keyword=' + queryJson),"); sb.AppendLine("             sqlkey: '"+ idColumn + "',//数据库主键");
                sb.AppendLine("             cols: [[");
                sb.AppendLine("                 //此处需修改");
                int cout = 1;
            ...
            else
            {
                ...
                sb.AppendLine("             url: '@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/GetGridJson',");
            ...
            sb.AppendLine("                     url: \"@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/Form\",");
            ...
            if (baseConfigModel.PageIndex.IsTree == 1)
            {
                sb.AppendLine("                 var entity = rendertree.checkStatus(false);");
            }
            else
            {
                sb.AppendLine("                 var entity = table.checkStatus('currentTableId').data;");
            }
            sb.AppendLine("                 if (entity.length == 0) {");
            sb.AppendLine("                     common.modalMsg(\"未选中数据\", \"warning\");");
            sb.AppendLine("                     return false;");
            sb.AppendLine("                 }");
            sb.AppendLine("                 var ids = [];");
            sb.AppendLine("                 for (var i = 0; i < entity.length; i++) {");
            sb.AppendLine("                     ids.push(entity[i].F_Id);");
            sb.AppendLine("                 }");
            sb.AppendLine("                 common.deleteForm({");
            sb.AppendLine("                     url: \"@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/DeleteForm\",");
            ...
            sb.AppendLine( "                   return false;");
            sb.AppendLine( "                }");
            sb.AppendLine("                if (entity.length > 1) {");
            sb.AppendLine("                   common.modalMsg(\"只能选择一条编辑\", \"warning\");");
            sb.AppendLine("                   return false;");
            sb.AppendLine("                 }");
            sb.AppendLine( "               common.modalOpen({");
            sb.AppendLine( "                  title: \"编辑界面\",");
            sb.AppendLine("                   url: \"@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/Form?keyValue=\" + entity.data." + idColumn + ",");
            sb.AppendLine("                   width: \"500px\",");
            sb.AppendLine("                   height: \"500px\",");
            sb.AppendLine( "               });");
            sb.AppendLine( "           }");
            ...
            sb.AppendLine("                 }");
            sb.AppendLine("                if (entity.length > 1) {");
            sb.AppendLine("                   common.modalMsg(\"只能选择一条查看\", \"warning\");");
            sb.AppendLine("                   return false;");
            sb.AppendLine("                 }");
            sb.AppendLine( "               common.modalOpen({");
            sb.AppendLine( "                  title: \"查看界面\",");
            sb.AppendLine("                   url: \"@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/Details?keyValue=\" + entity.data." + idColumn + ",");
            sb.AppendLine("                   width: \"500px\",");
            sb.AppendLine("                   height: \"500px\",");
            sb.AppendLine( "                  btn: []");
            sb.AppendLine( "               });");
        ...
        public string BuildForm(BaseConfigModel baseConfigModel)
        {
            ...
            sb.AppendLine("                   url: '@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/GetFormJson',");
            ...
            sb.AppendLine("               url: '@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/SubmitForm?keyValue=' + keyValue,");
        ...
        public string BuildMenu(BaseConfigModel baseConfigModel, string idColumn = "F_Id")
        {
            ...
            foreach (string btn in baseConfigModel.PageIndex.ButtonList)
            {
                ...
                string url = "@(YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory)/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/" + form;
        ...
        public async Task<List<KeyValue>> CreateCode(BaseConfigModel baseConfigModel, string code)
        {
            ...
            if (!param["CodeIndex"].IsEmpty())
            {
                ...
                string menuUrl = YMWeb.Code.GlobalContext.SystemConfig.VirtualDirectory +"/" + baseConfigModel.OutputConfig.OutputModule + "/" + baseConfigModel.FileConfig.ClassPrefix + "/" + baseConfigModel.FileConfig.PageIndexName;
