			...
			switch (layui.stope(e), F.trigger("table.tool.panel.remove"), l.close(i.tipsIndex), c) {
				...
				case "LAYTABLE_EXPORT":
					r.ie ? l.tips("导出功能不支持 IE，请用 Chrome 等高级浏览器导出", this, {
						...
						list: function () {
							return ['<li class="exportcsvAll"  data-type="csv">导出到 Csv 文件</li>', '<li class="exportAll" data-type="xls">导出当前页数据到 Excel 文件</li>', '<li class="exportSelected" data-type="xls">导出选中数据到 Excel 文件</li>'].join("")
						}(),
						done: function (e, l) {
							var checkStatus = d.checkStatus(a.id);
							l.on("click", function () {
								var e = t(this)
									.data("type");
								var choise = this.className;
								if (choise === 'exportSelected') {
									if (checkStatus.data.length < 1) {
										layer.alert('请选择至少一条记录进行导出');
										return false;

									} else {
										d.exportFile(a.id, checkStatus.data, e)
									}
								} else {
									d.exportFile(a.id, null, e)
                                }
