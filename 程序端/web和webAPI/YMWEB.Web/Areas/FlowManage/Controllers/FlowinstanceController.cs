...
using YMWeb.Service;
using YMWeb.Service.FlowManage;

namespace YMWeb.Web.Areas.FlowManage.Controllers
{
    /// <summary>
    /// 创 建：超级管理员
    ...
    public class FlowinstanceController :  ControllerBase
    {
        ...
        public async Task<ActionResult> Verification(VerificationExtend entity)
        {
            try
            {
                ...
                return await Success("操作成功。", className, entity, entity, entity.F_FlowInstanceId,DbLogType.Submit);
            }
            catch (Exception ex)
            {
                return await Error(ex.Message, className, entity, entity,entity.F_FlowInstanceId, DbLogType.Submit);
            }
        }
