...
namespace YMWeb.Web.Areas.SystemManage.Controllers
{
    ...
    public class AreaController : ControllerBase
    {
        ...
        public async Task<ActionResult> SubmitForm(AreaEntity areaEntity, string keyValue)
        {
            ...
            if (!string.IsNullOrEmpty(keyValue))
            {
                data = await _areaService.GetForm(keyValue);
            }
            ...
            try
            {
                ...
                return await Success("操作成功。", className, areaEntity, data, keyValue);
            }
            catch (Exception ex)
            {
                return await Error(ex.Message, className, areaEntity, data, keyValue);
            }
        }
        [HttpPost]
        [HandlerAjaxOnly]
