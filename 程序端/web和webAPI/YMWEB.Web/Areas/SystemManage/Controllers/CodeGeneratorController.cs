...
using YMWeb.Domain;
using YMWeb.Code;
using YMWeb.CodeGenerator;
using YMWeb.Service.CommonService;
using System.Linq;
using YMWeb.Service;
using System.Threading.Tasks;
using YMWeb.Code.Extend;
using Chloe;
    ...
    public class CodeGeneratorController : ControllerBase
    {
        ...
        public async Task<ActionResult> CodeGenerateJson(BaseConfigModel baseConfig, string Code)
        {
            try
            {
                ...
                return await Success("操作成功。", className, baseConfig, baseConfig, "");
            }
            catch (System.Exception ex)
            {
                return await Error(ex.Message, className, baseConfig, baseConfig, "");
            }
        }
        [HttpPost]
        [HandlerAjaxOnly]
