...
using YMWeb.Code;
using YMWeb.Domain.FileManage;
using YMWeb.Service;
using YMWeb.Service.FileManage;
using YMWeb.Service.SystemOrganize;
using System.Net.Http.Headers;
using System.IO;

namespace YMWeb.Web.Areas.FileManage.Controllers
{
    /// <summary>
    /// 创 建：超级管理员
    ...
    public class UploadfileController :  ControllerBase
    {
        ...
        public async Task<ActionResult> Upload(string fileby,int filetype =0)
        {
            ...
            try
            {
                ...
                await _logService.WriteLog("操作成功。",className, entity, entity, "",DbLogType.Visit);
                return Content(new { code = 0, msg = "操作成功", data = new { src = entity.F_FilePath, title = fileName } }.ToJson());
            }
            catch (Exception ex)
            {
                await _logService.WriteLog(ex.Message, className, entity, entity, "", DbLogType.Visit,true);
        ...
        public async Task<bool> SubmitForm(UploadfileEntity entity, string keyValue)
        {
            ...
            try
            {
                ...
                await Success("操作成功。", className, entity, data, keyValue);
                return true;
            }
            catch (Exception ex)
            {
                await Error(ex.Message, className, entity, data, keyValue);
                return false;
            }
        }
